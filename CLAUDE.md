...
## 🧪 AI分業とテスト原則（2026-05-27）

### 分業の基本

- **GPT / 対話本体**: 複雑な仕様判断、音楽的判断、UI/UXの意味づけ、PUSH実機の操作体系、リリース可否判断を担当する。複数の現象が絡む時は、ここで因果を整理してから実装へ渡す。
- **Codex / Claude Code**: 切り出せる実装、機械的な差分確認、テスト実行、スクリーンショット確認、配信整合性確認、反復的な表記・翻訳・マニュアル同期を担当する。
- **うりなみさん**: 実機の演奏感、音楽的自然さ、PUSHの手触り、資料由来フォームの実用性だけを見る。リロード、キャッシュ削除、開き直し、手作業の検証手順を背負わせない。

### Codex / Claude Code に任せるテスト

- `npm test`: 理論、コード判定、ギターseed、表記、データ整合性を機械的に確認する。
- `npx playwright test tests/e2e/deploy.spec.js`: Service Worker、キャッシュ、配信整合性を確認する。
- `BASE_URL=https://murinaikurashi.com/apps/64-pad npx playwright test tests/e2e/deploy.spec.js`: 本番反映後の配信整合性を確認する。
- 画面レイアウト変更時: Playwright screenshot で、パッドとビルダーの縮小、折り返し、表示揺れ、文字被りを確認する。
- 機能更新時: Help / Tutorial / Guide / オンラインマニュアル / 更新履歴 / 翻訳 / リリース前テスト項目の更新漏れを確認する。
- 新機能追加時: 「マニュアルに書くべき箇所」「書かなくてよい内部実装」「必要なスクショ/動画」「オンラインマニュアル用MD草案」をCodex/Claude Code側で作る。うりなみさんは最終的な表現、体験の言い方、不要なAI臭さの削除に集中する。

### Codex への具体指示

- ユーザーに「リロードして」「開いて」「キャッシュを消して」と言わない。Browser / Playwright / Computer Use で可能な確認は Codex 側で完結する。
- UIの見た目確認は Browser / Playwright screenshot を第一選択にする。Macアプリ、Desktop版、PUSH関連の画面確認などブラウザ外の操作が必要な時だけ Computer Use を使う。
- Computer Use で潰せるもの: Desktop版の起動確認、ウインドウ表示、ボタンやメニューの見え方、インストーラー/アプリ起動、簡単なクリック動作、画面上の文字被りやレイアウト崩れ。
- Computer Use で代替しないもの: PUSH実機のLED色、点滅の疲労感、演奏時の身体感覚、ギターフォームの実用性、音楽的な良し悪し。
- 画面が小さくなる、折り返す、パッドとビルダーが縮む、Note/Degree が被る、PUSH画面の行が動く、といった既知の再発は、スクリーンショット付きで確認してから完了扱いにする。
- デプロイ判断では、`npm test` と `deploy.spec.js` の結果を先に出す。audio E2E が落ちた場合は、今回差分由来か既存本番でも落ちるかを比較し、結論を分けて報告する。
- submodule 変更がある時は、`git status -sb` で親 repo / `pad-core` / `audio-core` をそれぞれ確認する。push は必ず submodule → 親 repo の順にする。
- 変更後は「何を確認したか」「何を確認できないか」「人間の実機確認が必要な項目」を短く分けて報告する。
- 機能変更後は、コード差分からユーザーに見える変更を抽出し、マニュアル反映候補を必ず作る。ユーザーにとってどうでもよい実装詳細はマニュアル草案に入れない。

### 新機能 → マニュアル草案パイプライン

機能追加・仕様変更・操作体系変更があったら、実装完了だけで止めない。Codex / Claude Code は次を作る。

1. **ユーザーに見える変更リスト**: 新しい操作、表示、ボタン割当、状態、制限、注意点。
2. **マニュアル反映判断**: 既存ページへ追記するか、独立章にするか、更新履歴だけで十分か。
3. **書かないことリスト**: 内部実装、ライブラリ名、ユーザー体験に関係しない技術説明、AI臭い抽象表現。
4. **必要素材**: スクショが必要な画面、PUSH実機写真が必要な状態、動画が向く操作。
5. **MD草案**: オンラインマニュアルに貼れる粒度で書く。ただし最終文体はうりなみさんが直す前提にする。
6. **うりなみさん確認用タスクリスト**: うりなみさんが最終判断すべき項目だけを、チェックできる粒度で出す。
7. **確認表**: 「反映済み」「草案のみ」「未確認」「実機確認待ち」を分ける。

マニュアル文は「何ができるか」「どう使うと体験が変わるか」を先に書く。技術的な仕組みは、必要な時だけ短く補足する。

### うりなみさん確認用タスクリストの必須項目

機能変更後、Codex / Claude Code は「確認してください」ではなく、判断しやすいタスクリストを作る。
...
特にレイアウト変更時は、既存マニュアル画像が古くなる。画面が変わったら「スクショ差し替え候補」を必ずタスクリストに入れる。

### GPT から Claude Code / Codex へ委譲する基準

- **GPTが保持する**: 仕様が未確定、音楽的・教育的判断が必要、PUSH実機の操作感が絡む、UIの意味づけが必要、ユーザー体験の言葉を決める作業。
- **Claude Codeへ渡す**: 大きなコード読解、既存仕様との差分調査、マニュアルやHelpの網羅チェック、実装候補の洗い出し、重複・矛盾・未反映箇所の発見。
- **Codexへ渡す**: ローカル実行、Playwright/Browser/Computer Use、スクリーンショット検証、テスト実行、コミット/デプロイ確認、狭く決まった修正。
- **Geminiへ渡す**: 長いマニュアル、公開ページ、FAQ、レビュー文の俯瞰・重複検出・読者視点の抜け検出。音楽的な最終判断は渡さない。
- **渡さない**: 「自然なボイシングか」「この操作が演奏中に疲れるか」「PUSHのLEDが気持ち悪いか」「教育用途として何を優先するか」。これは GPT + うりなみさんで判断する。

この環境には `claude` / `codex` / `gemini` CLI がある。必要に応じて、GPTが仕様と禁止事項を切ったプロンプトを作り、CLIまたはサブエージェントへ委譲する。ただし、委譲結果はそのまま採用せず、GPTが仕様・音楽性・ユーザー体験に照らして統合する。

### 委譲プロンプトの型

Claude Code / Codex へ依頼する時は、以下の形にする。広い相談ではなく、狭い検査・実装・確認へ切る。

```text
目的:
この作業で達成したいことを1〜3行で書く。

背景:
GPTとうりなみさんで決めた仕様判断を書く。音楽的・教育的理由がある場合は必ず書く。

SSOT:
読むべきファイル、信頼するファイル、触ってはいけない派生物を書く。

やってよいこと:
調査対象、編集対象、実行してよいテスト、スクリーンショット確認の範囲を書く。

やってはいけないこと:
仕様変更、音楽的判断、UI方針変更、ユーザーへの操作依頼、推測での削除を禁止する。

受け入れ条件:
テスト、表示、翻訳、マニュアル、スクリーンショット、未確認事項を具体的に書く。

出力:
変更点、確認済み、未確認、GPT/うりなみさん判断が必要な点だけを短く返す。
```
...
```text
目的:
PUSH 3 の色選択画面で、パッドにカラーパレットが表示されない再発を検査する。

背景:
色は64Pad Explorer本体ではなく、PUSHのパッドから選ぶ。設定画面に入った瞬間にPUSH側の表示も切り替わる必要がある。

SSOT:
/Users/nozakidaikai/64-pad-visualizer/CLAUDE.md
/Users/nozakidaikai/64-pad-visualizer/push3-controller.js
/Users/nozakidaikai/64-pad-visualizer/index.html

やってよいこと:
コード読解、Playwrightで画面確認、Computer UseでDesktop画面確認、必要ならスクショ取得。

やってはいけないこと:
PUSH実機のLED色の良し悪しを判断しない。ユーザーに再起動やリロードを頼まない。

受け入れ条件:
設定画面へ入った時点でPUSH画面更新処理が呼ばれること。色選択中に通常Scale表示へ戻らないこと。確認できない実機項目は明示すること。

出力:
原因、修正点、確認済み、実機確認が必要な点だけ。
```

### 人間が見るテスト

- PUSH 3 実機のLED色、点灯速度、点滅の疲労感、ボタンの自然さ。
- TASTY / STOCK / Guitar のボイシングが音楽的に自然か。
- ギターのミュート、フィンガリング、ジャンル別フォームの実用性。
- Desktop 版でのControl Surface操作、MIDIポート、実機連携。

### リリース判定

- 変更範囲に直接関係するユニットテストは必須で通す。
- `deploy.spec.js` は本番前後で必ず通す。
- audio E2E が既存不合格の時は、今回差分由来か既存課題かを切り分ける。既存課題ならリリースブロッカーにしないが、必ず記録する。
- submodule が ahead の時は **submodule を先に push → 親 repo を push**。順序を逆にしない。
- 実機でしか分からない項目は、Codexが代替したふりをしない。機械テストで潰せる範囲を潰してから、うりなみさんの確認負荷を最小化する。

---

## 現在地（自動更新）

- 状態: **2026-05-29 コードモード表示リデザインを V6.7.0 として本番公開**（version-tag V6.6.0→V6.7.0 = MINOR、whats_new_670 ja/en 追記＝他7言語は別タスク、main push → GitHub Actions deploy、オンラインマニュアルも同時 deploy）。元の 3 commit:
  - `b04437a` = 基本形デフォルト（コード選択で root position の 1 配置＋スケール背景を表示、全配置を一度に出さない）＋色言語（コード音=グレー `#a3a3a3` / スケール音=青 `#3f7aa6` / スケールルート=オレンジ `#E69F00`、度数・特性音の色は表示設定「特性音に色を付ける」opt-in で既定オフ）＋上下=転回（オクターブ越え・グリッド端で停止）/Shift+上下=オクターブ＋同 register 別配置の「1/N」バッジ（点滅廃止、最低音=ベースのパッドに表示）＋「全ポジション表示」を右 Voicing から左下フッターへ移設（Shift+F）＋全ポジション表示は破線枠廃止し ABCD ラベルのみ＋スケール背景表示＋白の instrument-highlight 枠抑止＋i18n 9言語（show_all_positions / scale_root）。
  - `003f908` = ダイアトニックバー選択時に発音（playCurrentChord）＋監査修正（基本形の転回上限 cap(3) 撤廃=5音以上のコードも全転回を辿る／未使用の点滅 CSS 削除／voicing_box ヘルプ文 9言語の「点滅バッジ」→「A1/3 件数」更新）。
  - `dd375a8` = パッド右クリックメニュー（メモリーに保存=Shift+C / 全ポジション表示=Shift+F / クリア=X、マウス操作のみ・タッチ非対応、グリッド上限定、左クリック発音は不変）。
  - **全 commit を独立 Claude フォールバック監査 = PASS**（Codex は rate-limit）、`npm test` 146 PASS、ブラウザ実機検証済。#13 半音バグは「基本形を毎回計算」方式で回避（解消）。
  - **audio-core submodule 注意（2026-05-29 デプロイ時発覚）**: `a1eb179` が audio-core を未push の `334d0b8`（試聴音 velocity 0.6）にバンプしていて Actions submodule checkout が失敗。さらに audio-core remote は別作業 `5797899`（spring routing 後段化+73鍵ゲート）に枝分かれ。`b747fc4` で audio-core を本番現行 `9bb8ac5` に固定（音変更ゼロ）して UI のみ公開。ローカル audio-core は detached HEAD `9bb8ac5`、velocity 0.6 は branch `wip/chord-audition-velocity-0.6` に退避。**audio-core = Web 版の音エンジン専用**（うりなみさん 2026-05-29 明言「Web とスタンドアローンでオーディオエンジンが違う、Web と分けないとダメ」）。Desktop/Standalone の音は C++ PluginProcessor（`juce::Synthesiser` + VST/AU host）= 別物で、audio-core は Desktop の音には一切使われない。sync-webui.sh は WebUI 一式（audio-core JS 含む）を Desktop に copy するが、Desktop の音は C++ なので audio-core JS は素通り（音には効かない）。**枝分かれ解消は Web 内で完結する話で、Standalone には波及しない**（耳判定必須、blind bump 禁止）。
- 状態(Perform/2026-05-31): **Perform 表示の教育的修正は Codex が `acf0827 Fix Perform educational display` として commit/deploy 済み**。正しい仕様 = 緑（`rgba(42,110,42,0.7)`）は Perform のどのモードでも使わない / コードトーン=常にグレー（`var(--pad-basic-chord)`）/「再生時に同一音を表示」(トグル ON)=録音した実ピッチ `slot.midiNotes` のみグレー表示・オクターブ違いコピー含めず。音のパスは絶対不変（`playMidiNotes(slot.midiNotes, 1.0)`、表示モードに左右されない）。2026-05-31 後続作業として、未コミットで Guitar engine 実音域表示、Chord モード pad クリックの入力化停止、楽器クリック Input 遷移、楽器入力 Clear/Play 廃止、Perform Push SWAP、表示設定 colorOff のギター/ベース/鍵盤反映を実装中。
- 残作業: **Desktop への `sync-webui.sh` 反映 + cmake rebuild**（公開済なので次の Desktop ビルドで取り込む）。whats_new_670 の他7言語翻訳。文字だけマニュアルページの画像補充＋Push 画面変更に伴う画像差し替え（**Push 実装完了後**）。うりなみさんによるマニュアル人力レビュー。#6 Push LED 色ユーザー選択、#7 Web パッド色パレット選択、#9 ショートカット/メニュー項目の洗い出し。
- 正規ルール: 理論 SSOT は `pad-core/theory.js` / `pad-core/data.js`。Web で修正 → Desktop は `sync-webui.sh` 経由、`WebUI/` は生成物。**main への push = GitHub Actions (`.github/workflows/deploy.yml`) = 本番 deploy（murinaikurashi.com、実ユーザーに即反映）。「push≠deploy」は誤り**（2026-05-29 訂正、本番は live service）。手動 deploy は `./deploy.sh`（rsync）。main push は pre-push hook が `DEPLOY_CONFIRMED=1` を要求（本番直結の安全装置）。コミット時 pre-commit hook が SW キャッシュ版数 auto-bump。確認サーバーは `_nocache_server.py`。うりなみさんの確認は別ブラウザ（127.0.0.1:8770）へ osascript で版数反映（手を使わせない）。機能更新時は Help/Tutorial/Guide・オンラインマニュアル・更新履歴・リリース前テスト・翻訳を同時更新。コードモード表示の設計 SSOT = `docs/basic-form-spec.md`。
- 次: **Push 3 の未対応部分**（SSOT = `Obsidian/デジタル百姓総本部/AI関連/調査/64Pad Explorer Push Control Surface 仕様メモ.md` の実機 verified mapping）。その前にマニュアル 5 章のスクショ撮影＋メタマニュアル整備を完了させる。
- 注意: ブラウザ版/Web版/VST3・AU で Push 3 を操作できると誤解させない（Desktop 専用）。auto-bump の SW キャッシュ版数（6.6.x）と version-tag（V6.7.0）は別系統。右クリックメニューは「形態」でなく「マウス操作（タッチ非対応）」で限定（デスクトップのブラウザ版でも動く）。HPS の TASTY は採譜・実用データで通常理論と完全一致しないことがある。
- 判断待ち: v1.5.0 Desktop 正式リリース日、Gumroad を英語中心にするか日本語本文+英語併記にするか、pad-core submodule の未 commit 整理。

### 2026-05-30 session 終盤の追記（Desktop PUSH regression 引継ぎ）
- 本 session で **Sonnet agent (tutorial 書き直しタスク)** が Web SSOT に uncommitted change を多数入れた（index.html / main.js / lang-* 9 言語 / style.css / tutorial.js / tutorial-data.js / pad-core submodule の中の CLAUDE.md）。これは Desktop 側の sync-webui.sh で Desktop bundle に embed される。
- 本 session で **Desktop 側で PUSH regression を導入してしまった**（mode 切替不能 / 機能キー LED 不点灯 / Push pad chord 表示なし / V6.72 表示等）。詳細引継書: `Obsidian/デジタル百姓総本部/AI関連/引継書/2026-05-30_64PE_session_aborted_push_regression.md`。
- Web SSOT は現状 stash pop 後の状態 = Sonnet change 含む。 必要なら別 session で再度 stash 化可能。 「巻き戻し」 は次 session で禁止。
- session 中 render.js と builder.js に一時 patch を入れたが両方 revert 済（= 現状 Web SSOT に私の patch 残骸なし）。

### 2026-05-31 Codex 完了記録（Perform 一ポジション/同一音 表示リワーク）
- **完了**: Web SSOT の Perform 表示を修正し、commit `acf0827` (`Fix Perform educational display`) 済み。`./deploy.sh` で本番 `https://murinaikurashi.com/apps/64-pad/` へ反映済み。公開 HTML は `V6.7.12` / `render.js?v=6.7.12` / `midi.js?v=6.7.12` を確認済み。
- **正しい仕様**: Perform では緑を使わない。コードトーン/表示音は常にグレー（`var(--pad-basic-chord)`）。背景には Scale/Chord と同じスケール面（スケール音=青 `--pad-basic-scale`、スケールルート=オレンジ `--pad-root`）を常時描く。トグル OFF は録音されたコードを教育用の一ポジションに畳んで表示、トグル ON（「再生時に同一音を表示」）は録音された実 MIDI ピッチだけをグレー表示し、オクターブ違いコピーは表示しない。音は常に `slot.midiNotes` をそのまま再生する。
- **真因**: Web 側の render だけでなく、Desktop/Push 側にも旧表示経路 `highlightPlaybackPads(slot.midiNotes)` が残っていた。Web SSOT では `midi.js` の `highlightPlaybackPads()` に `memoryViewMode === 'perform'` ガードを入れ、Perform では旧緑 overlay を絶対に出さないようにした。Desktop 側はまだ未コミットだが、Standalone 実機でこの修正により緑とテンション表記の問題が同時に解消した。
- **主な変更ファイル**: `render.js`（Perform/Input の教育用色・一ポジション/同一音表示・常時スケール背景）、`midi.js`（Perform で旧緑 overlay 無効化）、`perform.js` / `plain.js`（再生表示経路整理）、`data.js` / `main.js` / `index.html` / `style.css` / `lang-*` / `sw.js`（トグル・文言・キャッシュ V6.7.12）。
- **検証**: `npm test` 146 PASS、`git diff --check` PASS、本番 URL で `V6.7.12` 確認。うりなみさんの Standalone 確認で「直った。テンション表記も一気に解決」。
- **残す注意**: Desktop repo は `Source/PluginEditor.cpp` / `Source/PluginProcessor.cpp` / `CLAUDE.md` に別件混在の未コミットあり。Web の Perform 修正は固定済みなので、Desktop は残バグを潰してから、Perform 表示分と Push/Page/To DAW 等の別件を分けて整理する。次の新機能候補は Memory/Perform slot の SWAP（長押し中のスロットへ別パッドを押したら入れ替え）。
...
- 単一キー = ボイシング操作（O/S/D）
- 表示切替系も、できるだけ機能名・表示名の頭文字に寄せる（G=Guitar, B=Bass, P=Piano, D=Secondary Dominant など）。単なる空きキーで割り当てない
- 頭文字が衝突する場合は、先に機能名・表示名・マニュアル上の呼び方を見直し、意味から連想できる代替キーを選ぶ。衝突回避だけの任意キーは最後の手段
...
## CHS 系統廃止 (2026-05-17 確定、うりなみさん明言)

**「CHS 系統は今後も入れない」** (うりなみさん 2026-05-17)

**背景**: Chordcat / Chordcat Manager (= AlphaTheta) はバグだらけで、コードセット保存自体が機能しない (= うりなみさん検証済、2026-02-25 で AlphaTheta に動画付き報告済、修正されないまま放置)。CHS フォーマット (.chs 4096 bytes binary) を 64PE が export しても、Chordcat 側で読めない / 保存できないため実用上 意味なし。

**削除済 (V6.1.1 = 2026-05-17、commit `7260c38`)**:
- `index.html`: CHS Export / CHS Import button 3 個 (= Plain mode + Memory panel)
- `plain.js`: `exportPlainChs()` / `importChs()` / `parseChsToSlots()` 関数 3 個

**注意**: `lang-*.js` 9 言語の `chs_all` translation key は **残置** (= 害なし、削除コスト > 残置コスト、9 lang 一括 sed の paradigm 違反 risk 回避)。

**今後**: CHS 機能の **復活禁止**。Chordcat 側の bug が解消されない限り対応しない paradigm。
