...
- 状態: **Web v6.5.13 相当の作業はローカルでコミット済みだが未push/未本番deploy**。親repoは `origin/main` より19コミット ahead、`pad-core` は7コミット ahead。直近の主な変更は、HPS の TASTY / STOCK / Guitar をコードビルダー内に整理、Push 3 操作・LED・表示調整、Guitar/Bossa/Jazz/Neo Soul/Folk の資料由来voicing seed追加、6系統表記整理、ヘッダーリンク整理、AI分業とテスト原則の追加。`CLAUDE.md` には未コミット差分あり。
- 残作業: **Web本番deploy判断/push**、**Desktop v1.5.0 RC/インストーラー確認**、**Push 3 実機テスト**、**Gumroad 商品ページ文言の差し替え**、**ブログの 64Pad Explorer / Push 3 操作説明反映**、**画像・動画を含むマニュアル強化**、**アプリ横断テストマニュアルと64Pad Explorer専用テストマニュアル草案作成**。Guitarエンジンは改善済みだが、Cornell Dupree系のOld-school R&B/Soul seed、ミュート/フィンガリングロジックは次の改善候補。
- 正規ルール: 理論判断の SSOT は `pad-core/theory.js` / `pad-core/data.js`。Web で修正 → Desktop は `sync-webui.sh` 経由で同期。Desktop 側 `WebUI/` は生成物として扱う。i18n は見える文言を全言語ファイルに追加する。機能更新時は Help/Tutorial/Guide、オンラインマニュアル、更新履歴、リリース前テスト、翻訳を同時更新する。新機能後は「マニュアル反映判断」「必要スクショ/動画」「MD草案」「うりなみさん確認用タスクリスト」を必ず作る。PWA のため確認サーバーは必ず `_nocache_server.py` を使う。
- 次: まず `CLAUDE.md` のAI分業/テスト運用追記を確認してコミットする。Web deploy するなら **`pad-core` を先に push → 親repoを push**。deploy後は `BASE_URL=https://murinaikurashi.com/apps/64-pad npx playwright test tests/e2e/deploy.spec.js --project=chromium` を実行する。別インスタンスには、アプリ横断テストマニュアル + 64Pad Explorer専用テストマニュアルの草案作成を渡す。
- 注意: audio E2E は現行本番でも既存不合格があるため、今回差分のブロッカーにするかは必ず切り分ける。2026-05-27時点で `npm test` は146 tests passed、ローカル `deploy.spec.js` は2 passed / 1 skipped、本番E2Eはdeploy系通過・audio系2件既存失敗。ブラウザ版/Web版・VST3/AU plugin では Push 3 Control Surface を操作できると誤解させない。Push 3 の Setup で表示設定を開けるのは Desktop 版だけ。PUSH2 は完全対応ではなく、ディスプレイ表示と部分的対応。6系統は `6(9)` 系で表示し、Lydian 系は `#11` として扱う。HPS の TASTY は採譜・実用データなので、通常のコードビルダー理論と完全一致しないことがある。
- 判断待ち: Web版をこのまま本番deployするか、v1.5.0 の正式リリース日、RC を誰に先行確認してもらうか、Gumroad を英語中心にするか日本語本文+英語併記にするか。テストマニュアル作成は別インスタンスに渡す方針。
